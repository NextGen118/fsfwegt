...
const EditdBilloflandingsubs = (props) => {
    ...
    const { id } = useParams()
    const [values, setValues] = useState({ date: '',seal_no:'',marks:'',package_quantity:'',description:'',gross_weight:'',measurement:'',
    ...
    const getBilloflandingsubsByid = () => {
        axios.get(`http://127.0.0.1:8000/api/billoflandingsubs/show/all`)
            .then(res => {
    ...
    const submitEdit = () => {
        axios.post(`http://127.0.0.1:8000/api/billoflandingsubs/store?date=${values.date}&bill_of_landing_id=${billoflandingselect}
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={12}>
                    <PageTitle
                        breadCrumbItems={[
                            { label: 'Bill of landing subs', path: '/billoflandingsubs' },
                            { label: 'Edit bill of landing sub', path: '/edit-billoflandingsubs/:id', active: true },
                        ]}
                        title={'Edit Bill of landing sub'}
                    />
                </Col>
            </Row>
