...
const EditdBilloflandingsubs_non_inventories = (props) => {

    const { id } = useParams()
    const [values, setValues] = useState({ seal_no:'',marks:'',package_quantity:'',description:'',gross_weight:'',measurement:'',
    ...
    const getBilloflandingsubsByid = () => {
        axios.get(`http://127.0.0.1:8000/api/billoflandingsubnoninventories/show/all`)
            .then(res => {
    ...
    const submitEdit = () => {
        axios.post(`http://127.0.0.1:8000/api/billoflandingsubnoninventories/store?bill_of_landing_id=${billoflandingselect}
        ...
        &other_expenses=${values.other_expenses}&other_expenses_remarks=${values.other_expenses_remarks}&deleted=${values.deleted}&id=${id}`)
            .then(res => {
                history.push('/billoflandingsubnoninventories')
            })
            .catch((error) => {
                console.log(error);
            });
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={12}>
                    <PageTitle
                        breadCrumbItems={[
                            { label: 'Bill of landing sub non inventories', path: '/billoflandingsubnoninventories' },
                            { label: 'Edit bill of landing sub non inventory', path: '/edit-billoflandingsubnoninventories/:id', active: true },
                        ]}
                        title={'Edit Bill of landing sub non inventory'}
                    />
                </Col>
            </Row>
...
export default EditdBilloflandingsubs_non_inventories;