...
const EditdBilloflandings_switches = (props) => {

    const { id } = useParams()
    const [values, setValues] = useState({
        date: '', bill_of_landing_number: '', export_references: '', detention_free_days: '', detention_description: '',
        ...
        hide_shipped_date: '', status_1: '', status_2: ''
    });
    ...
    const getIgm = () => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/igmindiavoyages/show/all`)
            .then(res => {
    ...
    const getBookingconfirmation = () => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/bookingconfirmations/show/all`)
            .then(res => {
        ...
        axios.get(`${process.env.REACT_APP_BASE_URL}/billoflandingswitches/show/all`)
            .then(res => {
                ...
                setValues({
                    ...
                    hide_shipped_date: data[0].hide_shipped_date,
    ...
    const submitEdit = () => {
        axios.post(`${process.env.REACT_APP_BASE_URL}/billoflandingswitches/store?date=${values.date}
        ...
        &port_id_final_dest=${port_finaldestselect}
        ...
        &hide_shipped_date=${values.hide_shipped_date}&status_1=${values.status_1}&status_2=${values.status_2}&id=${id}`)
            .then(res => {
                history.push('/billoflandingswitches')
            })
            .catch((error) => {
                console.log(error);
            });
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={12}>
                    <PageTitle
                        breadCrumbItems={[
                            { label: 'Bill of lading switch', path: '/billoflandingswitches' },
                            { label: 'Edit bill of lading switch', path: '/edit-billoflandingswitches/:id', active: true },
                        ]}
                        title={'Edit Bill of lading switch'}
                    />
                </Col>
            </Row>
                            ...
                            <Row>
                                ...
                                <Col lg={6}>
                                    <AvForm>
                                        ...
                                        <AvField name="hide_shipped_date" label="Hide shipper date" type="text" required onChange={handleChange} value={values.hide_shipped_date} />
...
export default EditdBilloflandings_switches;